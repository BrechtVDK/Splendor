...
public class EindeSpelScherm extends VBox {
	...
	public EindeSpelScherm(List<String> lijstWinnaars, Hoofdscherm hs) {
		...
		sbWinaars.append(lijstWinnaars.stream().limit(lijstWinnaars.size() - 1).collect(Collectors.joining(", "))) //$NON-NLS-1$
				.append(Taal.vertaling("EindeSpelScherm.1")).append(lijstWinnaars.get(lijstWinnaars.size() - 1)); //$NON-NLS-1$
	...
	private void buildGui() {
		...
		lblWinnaars.setText(String.format(Taal.vertaling("EindeSpelScherm.5"), winnaars, eenWinnaar ? Taal.vertaling("EindeSpelScherm.6") : Taal.vertaling("EindeSpelScherm.7"))); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
