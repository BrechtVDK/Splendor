...
public class WelkomScherm extends VBox {
	...
	private void startGeklikt(ActionEvent evt) {
		...
		/*
		 * Stage stage = (Stage) this.getScene().getWindow(); double breedte =
		 * this.getScene().getWidth(); double hoogte = this.getScene().getHeight();
		 * stage.setTitle("Registratie spelers");
		 * 
		 * Scene scene = new Scene(new SpelerRegistratieScherm(dc, this), breedte,
		 * hoogte); stage.setScene(scene);
		 */
